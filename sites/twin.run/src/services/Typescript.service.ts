import { LSPConfig, TypeScriptProgram } from '@native-twin/language-service';
...
export const TypescriptContextLive = Effect.gen(function* () {
  ...
  const project = new ts.Project({
    compilerOptions: ts.ts.getDefaultCompilerOptions(),
    useInMemoryFileSystem: true,
    fileSystem: undefined,
    skipAddingFilesFromTsConfig: true,
    skipLoadingLibFiles: true,
  });
  // const compilerOptionsRef = yield* Ref.make<ts.CompilerOptions>(ts.ts.getDefaultCompilerOptions());
