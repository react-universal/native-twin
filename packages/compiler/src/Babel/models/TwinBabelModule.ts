...
import { TwinJSXElement } from './TwinJSXElement';
import { TwinJSXElementNode } from './TwinJSXElementNode';
      ...
      const tree = makeTreeFrom({
        ...
        transform: (jsxElement) => {
          ...
          const dependency = Option.fromNullable(
            jsxElement.scope.getBinding(ident.name),
          ).pipe(
            Option.andThen(getBabelBindingImportSource),
            Option.map((importSource) =>
              TwinPath.NodePath.resolve(file.dirname, importSource.source),
            ),
            Option.andThen((resolvedPath) =>
              RA.findFirst(dependencies, (x) => x.filepath.startsWith(resolvedPath)),
            ),
          );
          return new TwinJSXElementNode(file, jsxElement, ident.name, dependency);
        },
      });
