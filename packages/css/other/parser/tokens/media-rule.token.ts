...
import type { Units, Context, PartialStyle, Transform } from '../css.types';
...
function convertAnyValue(
  key: keyof Context | keyof PartialStyle | keyof Transform,
  value: string,
  units: Units,
) {
  if (key === 'resolution') {
  ...
  return (Object.keys(constraint) as (keyof Constraint)[]).every((key) => {
    ...
    if (minMax === 'Min') {
      ...
      if (baseKey.endsWith('aspectRatio'))
        return (
          Math.abs((context[baseKey] as number) - (value as number)) <
          ((value as number) + (context[baseKey] as number)) / 100
        );
      return context[baseKey] === value;
    }
  });
