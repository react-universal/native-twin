...
export const getSheetDeclInfo = (decl: SheetEntryDeclaration) => {
  ...
  return { kind, valueType, supportsAuto, supportsNone, isUnknownDecl, isUnitless };
};

export const parseSheetDecl = (decl: SheetEntryDeclaration, ctx: CreateStylerInput) => {
  const kind = getDeclKind(decl.prop);
  const valueType = getDeclValueKind(decl.prop);
  const supportsAuto = PropPredicates.supportsAuto(decl.prop);
  const supportsNone = PropPredicates.supportsNone(decl.prop);
  ...
  return { value, decl, isUnitless, isUnknownDecl, supportsAuto, supportsNone, valueType, kind };

  function guessDeclValue() {
    if (isUnknownDecl) unknownValue(decl.value);
    if ((supportsAuto && decl.value === 'auto') || (supportsNone && decl.value === 'none')) {
      return compiledValue(decl.value);
    }
    if (isUnitless && (typeof decl.value === 'number' || typeof decl.value === 'string')) {
      const result = Number.parseInt(decl.value.toString());
...
const withParseStyleAuto = <A>(parser: P.Parser<A>) => P.choice([P.literal('auto'), parser]);
const withParseStyleNone = <A>(parser: P.Parser<A>) => P.choice([P.literal('none'), parser]);
