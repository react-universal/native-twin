...
> = T extends `${infer Head}${Delimiter}${infer Tail}` ? [Head, ...Split<Tail, Delimiter>] : [T];

// Special parser to break down dot + bracket notation (e.g., "users[0].name")
type ParsePath<T extends string> = T extends `${infer Head}[${number}]${infer Rest}`
  ? [Head, number, ...ParsePath<Rest extends `.${infer R}` ? R : Rest>]
  : Split<T, '.'>;

type ResolvePath<T, Parts extends readonly any[]> = Parts extends [infer Head, ...infer Rest]
