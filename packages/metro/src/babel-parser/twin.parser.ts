...
import { RuntimeTW, parseCssValue } from '@native-twin/core';
import {
  AnyStyle,
  FinalSheet,
  SheetEntry,
          ...
          JSXAttribute: (attribute) => {
            if (attribute.node.name.name === 'className') {
              if (t.isStringLiteral(attribute.node.value)) {
                ...
                const ast = template.expression(JSON.stringify(sheet.base));
                const newAttribute = t.jsxAttribute(
                  t.jsxIdentifier('style'),
                  t.jsxExpressionContainer(ast()),
                );
...
function getSheetEntryStyles(entries: SheetEntry[] = [], tw: RuntimeTW) {
