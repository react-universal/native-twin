...
const oneOfType = (tokenType: (...args: any[]) => any) => (functionStream: TokenStream) => {
...
const xyTransformFactory =
  (tokenType: (...input: any) => any) =>
  (key: string | null, valueIfOmitted?: number | string) =>
  (functionStream: TokenStream) => {
...
export default (tokenStream: TokenStream) => {
  ...
  while (tokenStream.hasTokens()) {
    ...
    if (typeof functionName === 'string' && functionName in partTransforms) {
