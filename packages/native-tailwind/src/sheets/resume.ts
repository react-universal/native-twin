...
export function getSheet(
  ...
  const sheet = typeof document === 'undefined' ? createVirtualSheet() : createCssomSheet();
  if (!disableResume) {
    sheet.resume = resumeSheet;
  }
...
export function stringify(target: unknown): string {
  ...
  return (
    ...
    ((target as CSSStyleSheet).ownerNode || (target as HTMLElement)).textContent ||
    ((target as CSSStyleSheet).cssRules
      ? Array.from((target as CSSStyleSheet).cssRules, (rule) => rule.cssText)
      : asArray(sheetEntriesToCss(target as SheetEntry[]))
    ).join('')
  );
}
