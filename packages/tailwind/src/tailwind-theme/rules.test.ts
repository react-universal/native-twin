...
test('group and peer marker classes', () => {
  ...
  assert.strictEqual(
    tw('peer~name-focus-visible:underline'),
    'peer~name-focus-visible:underline',
  );
  assert.deepEqual(tw.target, [
  ...
  assert.strictEqual(
    tw('peer-focus-visible/label:underline'),
    'peer-focus-visible/label:underline',
  );
  assert.deepEqual(tw.target, [
...
test('group and peer hashed marker classes', () => {
  ...
  assert.deepEqual(tw.target, [
    '.\\#1bk5mm5:hover .\\#1o9546u{text-decoration-line:underline}',
  ]);
  ...
  assert.deepEqual(tw.target, [
    '.\\#1krcwoi[disabled]~.\\#1tsl4h5{text-decoration-line:underline}',
  ]);
  ...
  assert.deepEqual(tw.target, [
    '.\\#ooal8[disabled] .\\#v0jdie{text-decoration-line:underline}',
  ]);
...
test('font-feature-settings values can be retrieved', () => {
  ...
  assert.deepEqual(tw.target, [
    `.css\\#13n96aj{font-family:Inter;font-feature-settings:'cv11'}`,
  ]);
});
