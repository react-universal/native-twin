...
function isAfter(left: vscode.Position, right: vscode.Position): boolean {
  return (
    right.line > left.line || (right.line === left.line && right.character >= left.character)
  );
}
...
export class StyledTemplateLanguageService implements TemplateLanguageService {
  ...
  private translateDiagnostic(
    ...
    const length =
      context.toOffset(
        this.virtualDocumentFactory.fromVirtualDocPosition(diagnostic.range.end),
      ) - start;
  ...
  private translateHover(
    ...
    return {
      ...
      textSpan: {
        ...
        length: hover.range
          ? context.toOffset(
              this.virtualDocumentFactory.fromVirtualDocPosition(hover.range.end),
            ) - start
          : 1,
      },
      displayParts: [],
...
function filterScssCompletionItems(items: vscode.CompletionItem[]): vscode.CompletionItem[] {
  return items.filter(
    (item) =>
      item.kind === vscode.CompletionItemKind.Function && item.label.substr(0, 1) === ':',
  );
}
