...
export class StyledTemplateLanguageService implements TemplateLanguageService {
  ...
  public getCompletionsAtPosition(
    ...
    const wrapper =
      this.virtualDocumentFactory.getVirtualDocumentWrapper(context);
  ...
  public getCodeFixesAtPosition(
    ...
    return this.translateCodeActions(
      context,
      this.scssLanguageService.doCodeActions(
        doc,
        range,
        { diagnostics },
        stylesheet,
  ...
  private toVsRange(
    ...
    return {
      start: this.virtualDocumentFactory.toVirtualDocPosition(
        context.toPosition(start),
      ),
      end: this.virtualDocumentFactory.toVirtualDocPosition(
        context.toPosition(end),
      ),
    };
  }
  ...
  private getCompletionItems(
    ...
    const emmetResults =
      emmetDoComplete(
        ...
        virtualPosition,
        'css',
        this.configurationManager.config.emmet,
      ) || emptyCompletionList;
    const completionsCss =
      this.cssLanguageService.doComplete(doc, virtualPosition, stylesheet) ||
      emptyCompletionList;
    const completionsScss =
      this.scssLanguageService.doComplete(doc, virtualPosition, stylesheet) ||
      emptyCompletionList;
  ...
  private translateDiagnostic(
    ...
    const code =
      typeof diagnostic.code === 'number' ? diagnostic.code : cssErrorCode;
    return {
      code,
  ...
  private translateCodeActions(
    ...
    for (const vsAction of codeActions) {
      ...
      const edits =
        vsAction.arguments && (vsAction.arguments[2] as vscode.TextEdit[]);
      if (edits) {
        actions.push({
          description: vsAction.title,
          changes: edits.map((edit) =>
            this.translateTextEditToFileTextChange(context, edit),
          ),
        });
      }
    }
...
function translateCompletionItemsToCompletionInfo(
  ...
  return {
    ...
    entries: items.items.map((x) =>
      translateCompetionEntry(typescript, x, doc, wrapper),
    ),
  };
}
