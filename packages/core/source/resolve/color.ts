import { TwColors } from '../types/tw-config';
import { ColorStyleType, isObject, isString, Style, StyleIR } from '../types/types';
import { warn, complete } from '../utils/helpers';

export function color(type: ColorStyleType, value: string, config?: TwColors): StyleIR | null {
  if (!config) {
    return null;
  }
