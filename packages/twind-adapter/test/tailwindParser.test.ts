...
describe('TailwindCSS compiler', () => {
  ...
  it('Normal color', () => {
    ...
    expect(css).toStrictEqual('/*!dbgidc,w,bg-black*/.bg-black{--tw-bg-opacity;background-color}')
  });

  it('Color with opacity', () => {
    const { css } = transformClassNames('bg-black/50');
    expect(css).toStrictEqual('/*!dbgidc,w,bg-black/50*/.bg-black\\/50{--tw-bg-opacity;background-color}')
  });
