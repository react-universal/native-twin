...
import type { TypescriptUtils } from '../TypescriptUtils.service';
 ...
 */
export const createTraversalContext = Effect.fn(function* (
  ...
  const extractNodeInfo = (node: ts.Node): TwinGraphModel.NodeInfo => {
    ...
    const mappedProps = tsUtils.getJSXMappedProps(node);
