...
export class Processor<Theme extends __Theme__, Presets extends Preset<any>[] = Preset[]> {
  ...
  private insertEntry(entry: SheetEntry) {
  ...
  public run(tokens: StringLike) {
    ...
    const parsedClasses = Iterator.from(this.parseClassnames(tokens));
    for (const rule of this._config.rules) {
    }
    parsedClasses.map((parsedClass) => {
      const rule = this._context.r(parsedClass);
    });
  }
}
