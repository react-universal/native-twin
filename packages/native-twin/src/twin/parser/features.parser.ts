...
import { cornerMap, directionMap } from '../constants';
import { maybeNegativeParser, twArbitraryParser, twSegmentParser } from './common.parser';
import type { RuleHandlerToken } from './twin.tokens';
...
const defaultResolver = (pattern: string, patternParser: P.Parser<string>) => {
  if (!pattern.endsWith('-') || pattern.includes('|')) {
    return P.sequenceOf([maybeNegativeParser, patternParser, P.endOfInput]).map(
      (x): RuleHandlerToken => ({
        segment: { _tag: 'class-segment', value: x[1] },
        base: x[1],
        suffixes: [],
        negative: x[0],
      }),
